...
pub fn render_input_dialog(f: &mut Frame, app: &App) {
    ...
    let input = Paragraph::new(app.input.as_str())
        ...
        .block(
            Block::default()
                .title(Span::styled(" \u{f067} Add Feed URL ", Style::default().fg(PRIMARY)))
                .borders(Borders::ALL)
...
pub fn render_filter_dialog(f: &mut Frame, app: &App) {
    ...
    let title = match app.focus {
        Focus::Feeds => " \u{f002} Filter Feeds ",
        Focus::Items => " \u{f002} Filter Items ",
        Focus::Tags => " \u{f002} Filter Tags ",
        Focus::Reader => " \u{f002} Filter ",
    };
...
pub fn render_tag_editor(f: &mut Frame, app: &App) {
    ...
    let tag_input = Paragraph::new(app.tag_input.as_str())
        ...
        .block(
            Block::default()
                .title(Span::styled(" \u{f02b} Add Tag (Press Enter) ", Style::default().fg(SUCCESS)))
                .borders(Borders::ALL)
    ...
    if !app.editing_tags.is_empty() {
        ...
        let tags_display = Paragraph::new(lines)
            .block(
                Block::default()
                    .title(Span::styled(" \u{f02b} Current Tags (Tab/Arrows to select, Del to remove) ", Style::default().fg(SUCCESS)))
                    .borders(Borders::ALL)
                    .border_style(Style::default().fg(SUCCESS)),
            );
